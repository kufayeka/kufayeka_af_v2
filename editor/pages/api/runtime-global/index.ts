...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const includeInternal = String(req.query.includeInternal || "").trim();
    const qs = includeInternal ? `?includeInternal=${encodeURIComponent(includeInternal)}` : "";
    const upstream = await fetch(`${getRuntimeBaseUrl()}/api/global${qs}`, {
      method: "GET",
    });

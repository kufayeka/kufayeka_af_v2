...
import { mergeTemplate, toRecord } from "./EventTemplateNormalizer";
import {
...
export async function openEventFromTemplate(options: {
  ...
  const captured = autoCapture
    ? mergeCaptured(autoCapture, options.openOptions?.capturedDataOnOpen ?? null)
    : options.openOptions?.capturedDataOnOpen ?? null;
    ...
    if (result.closedCount > 0) {
      ...
      if (closedRow) {
        ...
        const childPatterns = Array.isArray((metadata.policy as { closeChildrenOnClosePatterns?: unknown })?.closeChildrenOnClosePatterns)
          ? ((metadata.policy as { closeChildrenOnClosePatterns?: unknown }).closeChildrenOnClosePatterns as unknown[])

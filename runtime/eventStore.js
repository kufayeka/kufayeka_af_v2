...
function toIsoTs(ts) {
  ...
  if (Number.isNaN(date.getTime())) {
    throw new Error(`Timestamp tidak valid: ${ts}`);
  }
  return date.toISOString();
}

function parseIsoTs(ts, fallback = null) {
...
function normalizeStatus(status) {
  ...
  if (!VALID_STATUS.has(value)) throw new Error(`Status tidak valid: ${status}`);
  return value;
}

function normalizeSeverity(severity) {
  const value = String(severity || "other").trim().toLowerCase();
  if (!VALID_SEVERITY.has(value)) {
    throw new Error(`Severity tidak valid: ${severity}`);
  }
  return value;
}
...
function normalizeContextFilters(contextFilters) {
    ...
    return {
      ...
      conditions: contextFilters.map((item) => {
        if (!item || typeof item !== "object") {
          throw new Error("Context filter array harus berisi object");
        }
        return {
          path: toJsonPath(item.path || item.key),
          operator: String(item.operator || item.op || "eq").toLowerCase(),
          value: item.value
        };
      })
    };
  ...
  if (typeof contextFilters === "object") {
    if (Array.isArray(contextFilters.conditions)) {
      const op = String(contextFilters.op || "AND").toUpperCase() === "OR" ? "OR" : "AND";
      const conditions = contextFilters.conditions.map((item) => {
        if (!item || typeof item !== "object") {
          throw new Error("Setiap context condition harus object");
        }
        return {
          path: toJsonPath(item.path || item.key),
          operator: String(item.operator || item.op || "eq").toLowerCase(),
    ...
    return {
      op: "AND",
      conditions: Object.entries(contextFilters).map(([key, value]) => ({
        path: toJsonPath(key),
        operator: "eq",
        value
      }))
...
function mapRow(row) {
  let parsedContext = {};
  try {
    parsedContext = row.context ? JSON.parse(row.context) : {};
  } catch {
    parsedContext = {};
  }
  return {
    ...
    end_ts: row.end_ts,
    status: row.status,
    severity: row.severity || "other",
    context: parsedContext,
    is_acknowledge: Boolean(row.is_acknowledge),
    acknowledged_ts: row.acknowledged_ts,
    notes_on_open: row.notes_on_open,
    notes_on_close: row.notes_on_close
  };
}
...
function applyMigrations(db) {
  ...
  if (!columnNames.has("severity")) {
    db.exec(`ALTER TABLE events ADD COLUMN severity TEXT NOT NULL DEFAULT 'other';`);
  }
  if (!columnNames.has("notes_on_open")) {
    db.exec(`ALTER TABLE events ADD COLUMN notes_on_open TEXT;`);
  }
  if (!columnNames.has("notes_on_close")) {
    db.exec(`ALTER TABLE events ADD COLUMN notes_on_close TEXT;`);
  }
  if (!columnNames.has("is_acknowledge")) {
    db.exec(`ALTER TABLE events ADD COLUMN is_acknowledge INTEGER NOT NULL DEFAULT 0;`);
  }
  if (!columnNames.has("acknowledged_ts")) {
    db.exec(`ALTER TABLE events ADD COLUMN acknowledged_ts TEXT;`);
  }
}

function buildContextWhere(contextFilters, params) {
...
function buildBaseWhere(query, params) {
...
function createEventStore(options = {}) {
  ...
  function open(eventPath, ts, context = {}, notesOnOpen = "", severity = "other") {
  ...
  function close(pattern = "*", ts, notesOnClose = "") {
    ...
    const stmt = db.prepare(`
      ...
      SET
        ...
        status = 'closed',
        notes_on_close = CASE
          WHEN ? IS NULL OR ? = '' THEN notes_on_close
          ELSE ?
        END
      WHERE status = 'open'
        AND event_path LIKE ? ESCAPE '\\';
    `);
  ...
  function closeById(id, ts, notesOnClose = "") {
  ...
  function acknowledgeById(id, ts) {
    const normalizedId = String(id || "").trim();
    if (!normalizedId) throw new Error("id wajib diisi");
    const normalizedTs = toIsoTs(ts);
    const result = db
      .prepare(
        `UPDATE events SET is_acknowledge = 1, acknowledged_ts = ? WHERE id = ?;`
      )
      .run(normalizedTs, normalizedId);
    return {
      id: normalizedId,
  ...
  function deleteByPattern(pattern = "*", status = "*", from = "*", to = "*", severity = "*") {
    const params = [];
    const whereSql = buildBaseWhere(
      { pattern, status, from, to, contextFilters: {}, severity },
      params
    );
  ...
  function query(
    ...
    const baseParams = [];
    const whereSql = buildBaseWhere(
      { pattern, from, to, status, contextFilters, severity },
      baseParams
    );
    ...
    const rows = db.prepare(rowSql).all(...rowParams).map(mapRow);
    const countSql = `SELECT COUNT(1) AS total FROM events ${whereSql};`;
    const countRow = db.prepare(countSql).get(...baseParams);
  ...
  function get(pattern = "*", from = "*", to = "*", status = "*", contextFilters = {}, options = {}) {
...
function ensureEventStore(runtime, options = {}) {
  const existing = runtime.getGlobal("eventStore");
  if (
    existing &&
    typeof existing === "object" &&
    typeof existing.open === "function" &&
